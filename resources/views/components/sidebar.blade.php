<div class="flex flex-col h-full w-60 px-4 py-6 justify-between bg-white dark:bg-gray-800 transition-colors duration-300">
    ...
    <div>
        ...
        <nav class="space-y-2">
            ...
            {{-- 🏠 Dashboard --}}
            <x-sidebar.link route="dashboard" label="Dashboard" icon="fas fa-home"
            ...
            <x-sidebar.dropdown icon="fas fa-user-shield" label="Admin" :active="request()->routeIs('admin.*')">
                ...
                {{-- 👥 User Management --}}
                <x-sidebar.dropdown icon="fas fa-users-cog" label="User Management"
                    :active="request()->routeIs('admin.users.*') || request()->routeIs('admin.roles.*')">
                    <x-sidebar.link route="admin.users.index" label="Users" icon="fas fa-user"
                        :active="request()->routeIs('admin.users.*')" />
                    <x-sidebar.link route="admin.roles.index" label="Roles" icon="fas fa-user-tag"
                        :active="request()->routeIs('admin.roles.*')" />
                </x-sidebar.dropdown>

                {{-- 📄 Daily Reports Fillable --}}
                <x-sidebar.dropdown icon="fas fa-file-alt" label="Daily Reports Management"
                ...
                <x-sidebar.dropdown icon="fas fa-car-crash" label="Accidental Reports"
                ...
                <x-sidebar.dropdown icon="fas fa-mountain" label="Natural Disaster Reports"
                    :active="request()->routeIs('admin.natural-disaster-fillable.*')">
                ...
                <x-sidebar.dropdown icon="fas fa-road" label="Road Closed Management"
                    :active="request()->routeIs('admin.road-closed-fillable.*') || request()->routeIs('admin.road-closed-reports.*')">
                    <x-sidebar.link route="admin.road-closed-fillable.index" label="Report Headers"
                        icon="fas fa-list" :active="request()->routeIs('admin.road-closed-fillable.*')" />
                    <x-sidebar.link route="admin.road-closed-reports.index" label="Reports"
                        icon="fas fa-file-alt" :active="request()->routeIs('admin.road-closed-reports.*')" />
                </x-sidebar.dropdown>
            ...
            {{-- 🧾 Assignments --}}
            <x-sidebar.dropdown icon="fas fa-tasks" label="Assignments"
                :active="request()->routeIs('admin.district-users.*')">
                <x-sidebar.link route="admin.district-users.index" label="District Wise User"
                    icon="fas fa-user-friends" :active="request()->routeIs('admin.district-users.*')" />
            </x-sidebar.dropdown>

            {{-- ⚙️ Resource Management --}}
            <x-sidebar.dropdown icon="fas fa-boxes" label="Resource Management"
                :active="request()->routeIs('admin.equipment.*') || request()->routeIs('admin.manpower.*') || request()->routeIs('admin.relief_material.*') || request()->routeIs('admin.deployments.*')">
                <x-sidebar.link route="admin.equipment.index" label="Equipment" icon="fas fa-cogs"
                ...
                <x-sidebar.link route="admin.relief_material.index" label="Relief Materials"
                    icon="fas fa-box-open" :active="request()->routeIs('admin.relief_material.*')" />
                <x-sidebar.link route="admin.deployments.index" label="Deployments" icon="fas fa-truck"
                    :active="request()->routeIs('admin.deployments.*')" />
            </x-sidebar.dropdown>

            {{-- 📊 Daily Reports --}}
            <x-sidebar.dropdown icon="fas fa-file-alt" label="Daily Reports"
                :active="request()->routeIs('admin.daily_reports.*') || request()->routeIs('admin.district-reports.*') || request()->routeIs('admin.daily_reports_dhams.*') || request()->routeIs('admin.reports.*') || request()->routeIs('admin.natural-disaster-reports.*')">
                <x-sidebar.link route="admin.daily_reports.index" label="Daily Reports"
                    icon="fas fa-calendar-day" :active="request()->routeIs('admin.daily_reports.*')" />
                ...
                <x-sidebar.link route="admin.reports.index" label="File Uploads"
                    icon="fas fa-upload" :active="request()->routeIs('admin.reports.*')" />
            ...
            <x-sidebar.link route="admin.pages.list" label="Pages" icon="fas fa-file-contract"
            ...
            <x-sidebar.link route="admin.settings.index" label="Settings" icon="fas fa-cog"
