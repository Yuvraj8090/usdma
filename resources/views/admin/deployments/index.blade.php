    ...
    <div class="py-6 container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex justify-between items-center mb-6">
            <div class="text-sm text-gray-500 dark:text-gray-400">
                <i class="fas fa-truck-moving mr-1"></i> Manage all deployments
            </div>
            <a href="{{ route('admin.deployments.create') }}" 
               class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-500 border border-transparent rounded-lg font-semibold text-white uppercase tracking-widest hover:from-indigo-600 hover:to-purple-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition-all duration-200 shadow-lg hover:shadow-xl">
                <i class="fas fa-plus-circle mr-2"></i> Add Deployment
            </a>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-700">
            ...
            <div class="px-6 py-4 bg-gradient-to-r from-indigo-500 to-purple-600">
                <h3 class="text-lg font-medium text-white flex items-center">
                    <i class="fas fa-truck-moving mr-2"></i> All Deployments
                </h3>
            </div>
            ...
            <div class="overflow-x-auto p-4">
                <x-table.data-table
                    ...
                    :headers="['ID', 'Resource', 'District', 'Location', 'Status', 'Deployed At', 'Returned At', 'Actions']"
                    :page-length="10"
                ...
                >
                    @forelse($deployments as $deployment)
                        <tr>
                            <td>{{ $deployment->id }}</td>
                            <td>
                                {{ class_basename($deployment->deployable_type) }} #{{ $deployment->deployable_id }}
                            </td>
                            <td>{{ $deployment->district?->name ?? '-' }}</td>
                            <td>{{ $deployment->location }}</td>
                            <td>
                                <span class="px-2 py-1 rounded text-xs font-semibold
                                    {{ $deployment->returned_at ? 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200' : 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' }}">
                                    {{ $deployment->returned_at ? 'Returned' : 'Deployed' }}
                                </span>
                            </td>
                            <td>{{ $deployment->deployed_at?->format('Y-m-d H:i') }}</td>
                            <td>{{ $deployment->returned_at?->format('Y-m-d H:i') ?? '-' }}</td>
                            <td class="text-center">
                                <!-- Edit Button -->
                                <a href="{{ route('admin.deployments.edit', $deployment->id) }}" 
                                ...
                                <!-- Delete Button -->
                                <form action="{{ route('admin.deployments.destroy', $deployment->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                                            ...
                                            onclick="return confirm('Are you sure you want to delete this deployment?')">
