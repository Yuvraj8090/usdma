    ...
    <div class="py-6 container mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <div class="text-sm text-gray-500 dark:text-gray-400">
                <i class="fas fa-layer-group mr-1"></i> Manage Categories (Human Dead, Men, Vehicles…)
            </div>
                        ...
                        @forelse($fillables as $fillable)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                ...
                                <td class="px-6 py-4 text-right space-x-2">
                                    <a href="{{ route('admin.daily_reports_fillable.edit', $fillable->id) }}"
                                       class="px-3 py-1 bg-yellow-500 text-white rounded-lg shadow hover:bg-yellow-600">
                                        <i class="fas fa-edit"></i> Edit
                                    </a>
                                    <form action="{{ route('admin.daily_reports_fillable.destroy', $fillable->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
