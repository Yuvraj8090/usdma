        ...
        <div
            ...
            <div class="px-6 py-4 bg-gradient-to-r from-indigo-500 to-purple-600">
                <h3 class="text-lg font-medium text-white flex items-center">
                    <i class="fas fa-file-alt mr-2"></i> All Uses
                </h3>
            </div>
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($users as $user)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-400">{{ $user->email }}</td>
                                <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-400">
                                    {{ $user->role->name ?? '-' }}</td>
                                <td
                                    class="px-3 py-2 text-sm 
                                      @if ($user->last_service_date && $user->last_service_date->isPast()) bg-red-500 text-white rounded 
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center px-6 py-4 text-gray-500 dark:text-gray-400">No
                                    users found.</td>
                            </tr>
                        @endforelse
                    </tbody>
