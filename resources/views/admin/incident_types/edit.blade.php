<x-app-layout>
    ...
    <div class="py-6 container mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
            <form action="{{ route('admin.incident-types.update', $incidentType->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Incident Type Name</label>
                    <input type="text" name="name" value="{{ old('name', $incidentType->name) }}"
                ...
                <div class="flex justify-end space-x-3">
                    <a href="{{ route('admin.incident-types.index') }}"
