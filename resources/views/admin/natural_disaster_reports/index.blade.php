    ...
    <div class="py-6 container mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <div class="text-sm text-gray-500 dark:text-gray-400">
                <i class="fas fa-globe-asia mr-1"></i> Manage natural disaster reports (counts grouped by category)
            </div>
            <a href="{{ route('admin.natural-disaster-reports.create') }}"
               class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-green-500 to-teal-500 rounded-lg font-semibold text-white hover:from-green-600 hover:to-teal-600 shadow-lg">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200">
                        ...
                        @foreach($reports->groupBy(['district_id','report_date']) as $districtReports)
                            @foreach($districtReports as $date => $rows)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    <td class="px-6 py-4">{{ $rows->first()->district->name ?? '-' }}</td>
                                    <td class="px-6 py-4">{{ \Carbon\Carbon::parse($date)->format('Y-m-d') }}</td>
                                    @foreach($parents as $parent)
                                        @foreach($parent->children as $child)
                                            @php
                                            ...
                                            @endphp
                                            <td class="px-6 py-4 text-center">{{ $count > 0 ? $count : '-' }}</td>
                                        @endforeach
                                    @endforeach
                                </tr>
                            @endforeach
                        ...
                        <tr class="bg-gray-100 dark:bg-gray-700 font-semibold">
                            ...
                            @foreach($parents as $parent)
                                @foreach($parent->children as $child)
                                    <td class="px-6 py-4 text-center">{{ $totals[$child->id] ?? 0 }}</td>
                                @endforeach
                            @endforeach
                        </tr>
                    </tbody>
